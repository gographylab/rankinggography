...
function PagePhoto({ id }) {
  ...
  React.useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === 'j' || e.key === 'ArrowDown') router.go(`/photo/${PHOTOS[Math.min(idx+1, PHOTOS.length-1)].id}`);
      if (e.key === 'k' || e.key === 'ArrowUp') router.go(`/photo/${PHOTOS[Math.max(idx-1, 0)].id}`);
    };
    ...
    <div className="page-fade">
      ...
      <div style={{ padding: '20px 0', borderBottom: '1px solid var(--rule)' }}>
        <div className="wrap mono" style={{ fontSize: 11, letterSpacing: '.14em', textTransform: 'uppercase', opacity: .65, display: 'flex', justifyContent: 'space-between' }}>
          <div>
            <Link to="/explore" style={{ opacity: .7 }}>Explore</Link>
            <span style={{ opacity: .35, margin: '0 12px' }}>/</span>
            <Link to={`/explore/${photo.cat.toLowerCase()}`} style={{ opacity: .7 }}>{photo.cat}</Link>
            <span style={{ opacity: .35, margin: '0 12px' }}>/</span>
            <span>{photo.id}</span>
          </div>
          <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 56, alignItems: 'start' }}>
            ...
            <div>
              ...
              <div style={{ marginTop: 56, padding: '32px 0', borderTop: '1px solid var(--rule)', borderBottom: '1px solid var(--rule)' }}>
                ...
                <div style={{ marginTop: 16 }}>
                  <Link to="/about-ranking" className="mono" style={{ fontSize: 11, letterSpacing: '.14em', textTransform: 'uppercase', opacity: .65, borderBottom: '1px solid var(--rule)', paddingBottom: 2 }}>
              ...
              <div style={{ marginTop: 56 }}>
                <SectionHeader title="ความเห็น" eyebrow={`${comments.length} comments`} />
                <div style={{ display: 'flex', gap: 12, marginBottom: 32 }}>
                  <input type="text" className="input" placeholder="พิมพ์ความเห็นของคุณ — ใช้ @ เพื่อ mention" style={{ flex: 1 }} />
                      ...
                      <div key={i} style={{ display: 'flex', gap: 16, paddingBottom: 24, borderBottom: '1px solid var(--rule)' }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                            <Link to={`/photographer/${c.user}`} style={{ fontSize: 13, fontWeight: 500, letterSpacing: '-.005em' }}>{cuser?.name || c.user}</Link>
            ...
            <aside style={{ position: 'sticky', top: 100 }}>
              ...
              <div style={{ padding: '28px 0', borderTop: '1px solid var(--fg)', borderBottom: '1px solid var(--rule)' }}>
                <Link to={`/photographer/${photographer.username}`} style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
              ...
              {more.length > 0 && (
                <div style={{ padding: '28px 0' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    {more.map(p => (
                      <div key={p.id} onClick={() => router.go(`/photo/${p.id}`)} style={{ cursor: 'pointer' }}>
      ...
      <section style={{ padding: '40px 0 80px' }} className="rule-top">
        <div className="wrap">
          <SectionHeader title="ภาพในแนวเดียวกัน" eyebrow={`More ${photo.cat}`} link={`/explore/${photo.cat.toLowerCase()}`} linkLabel="See category" />
